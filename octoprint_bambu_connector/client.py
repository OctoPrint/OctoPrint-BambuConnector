...
class BambuClientListener:
    def __init__(self):
        self._logger = logging.getLogger("octoprint.plugins.bambu_connector.listener")

    def on_bambu_connected(self) -> None:
        pass
    ...
    def on_bambu_print_detected(
        self,
        path: str,
    ) -> None:
        pass

    def on_bambu_server_info(self, server_info: dict[str, Any]) -> None:
        pass

    def on_bambu_printer_files_updated(self, files: list[FileInfo]) -> None:
        pass
    ...
    def on_bambu_idle_state(self, state: IdleState) -> None:
        pass

    def on_bambu_gcode_log(self, *line: str) -> None:
        pass
    ...
    def on_bambu_position_update(self, position: Coordinate) -> None:
        pass

    def on_bambu_printer_logs(self, message: str) -> None:
        self._logger.info(message)


BAMBU_STATE_ERROR_LOOKUP = {
    BambuState.STARTUP: "Bambu is still starting up",
    BambuState.ERROR: "Bambu experienced an error during startup",
    BambuState.SHUTDOWN: "Bambu is in a shutdown state",
    BambuState.DISCONNECTED: "Bambu is not running, has experienced a critical error during startup or doesn't have its API server enabled",
}
...
# TODO: update for BambuClient
class BambuClientConnector(BambuClient):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._bambu_state: BambuState = BambuState.UNKNOWN
        self._bambu_state_subscription = False
    ...
    @property
    def bambu_state(self) -> BambuState:
        return self._bambu_state

    @bambu_state.setter
    def bambu_state(self, value: BambuState) -> None:
        if value == self.bambu_state:
            return

        old_state = self.bambu_state

        self._bambu_state = value

        if old_state != BambuState.READY and value == BambuState.READY:
            self.attempt_handshake(reset=True)

    @property
    def current_temperatures(self) -> dict[str, TemperatureDataPoint]:
    ...
    def on_close(self, cls, code: int, reason: str):
        try:
            ...
            self._listener.on_bambu_disconnected(error=error)
        except Exception:
    ...
    def on_error(self, cls, exc: Exception) -> None:
        try:
            ...
            self._listener.on_bambu_disconnected(error=str(exc))
        except Exception:
    ...
    def identify_connection(
        ...
        def on_connection_identified(future: Future) -> None:
            try:
                ...
                self._listener.on_bambu_disconnected(error=error_str)
    ...
    def attempt_handshake(self, reset=False) -> None:
        ...
        if not self._bambu_state_subscription:
            ...
            self._bambu_state_subscription = True
        ...
        def on_server_info(future: Future) -> None:
            try:
                ...
                self._bambu_state = BambuState.for_value(
                    server_info.get("klippy_state", "unknown")
                )

                if self.bambu_state == BambuState.READY:
                    # proceed with connection
                    self._listener.on_bambu_server_info(server_info)
                    ...
                    # log error
                    error = BAMBU_STATE_ERROR_LOOKUP.get(self._bambu_state)
                    self._listener.on_bambu_gcode_log(f"!!! {error}")
    ...
    def subscribe_to_updates(self) -> None:
        ...
        def on_printer_objects_subscribed(future: Future) -> None:
            try:
                future.result()
                self._listener.on_bambu_connected()
    ...
    def send_gcode_script(self, script: str) -> Future:
        ...
        def on_result(future: Future) -> None:
            try:
                result = future.result()
                self._listener.on_bambu_gcode_log(f"<<< {result}")
            except Exception:
    ...
    def refresh_files(self, root="gcodes") -> Future:
        def on_result(future: Future) -> None:
            try:
                ...
                self._listener.on_bambu_printer_files_updated(self._current_files)
            except Exception:
        ...
        **kwargs,
        ...
        def upload(
            ...
            try:
                ...
                headers = {}
                if self._access_code:
                    headers["X-Api-Key"] = self._access_code

                url = (
                    self.HTTP_URL.format(host=self._host, port=self._serial)
                    + "/server/files/upload"
                )
    ...
    def on_gcode_response(self, _, params):
        ...
        for line in params:
            if line.startswith(ACTION_PREFIX):
                ...
                self._listener.on_bambu_action_command(
    ...
    def _update_temperatures(self, payload: dict[str, Any]) -> None:
        ...
        if dirty_actual or dirty_target:
            ...
            self._listener.on_bambu_temperature_update(self._current_temperatures)
    ...
    def _update_print_stats(self, payload: dict[str, Any]) -> None:
        ...
        if print_stats.state is not None:
            ...
            self._listener.on_bambu_printer_state_changed(printer_state)

        if (
            print_stats.total_duration is not None
            or print_stats.print_duration is not None
        ):
            self._listener.on_bambu_print_progress(
    ...
    def _update_idle_timeout(self, payload: dict[str, Any]) -> None:
        ...
        if idle_timeout.state is not None:
            ...
            self._listener.on_bambu_idle_state(state)
    ...
    def _update_gcode_move(self, payload: dict[str, Any]) -> None:
        ...
        if position.gcode_position is not None:
            self._listener.on_bambu_position_update(position.gcode_position)
...
if __name__ == "__main__":
    ...
    client = BambuClientConnector(HOST, listener=Listener(), daemon=False)
    client.connect()
