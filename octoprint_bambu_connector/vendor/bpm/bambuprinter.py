...
class BambuPrinter:
    ...
    def get_sdcard_contents(self):
        ...
        def search_for_and_remove_all_other_files(mask: str, entry: dict):
            if "children" in entry:
                entry["children"] = [
                    x["id"].endswith(mask) or "children" in x for x in entry["children"]
                ]
                for child in entry["children"]:
                    search_for_and_remove_all_other_files(mask, child)

        self._sdcard_3mf_files = copy.deepcopy(self._sdcard_contents)
    ...
    def _get_sftp_files(
        ...
        for entry in files:
            if entry.is_dir:
                if entry.path in self.IGNORED_DIRS:
                    continue
